    ...
    public partial class Form1 : Form
    {
        ...
        private void save()
        {
            ...
            if (overWorld)
            {
                if (usedspace > freespace)
                {
                    ...
                    {
                        if (!cSprite.Checked)
                            save = mapSaver.saveOverworldCollision(overworldDrawer.objects, overworldDrawer.warps, mapIndex, floorTile, wallTiles, cSpecialTiles.Checked, usedspace, freespace, cSpecialTiles.Checked, overworldDrawer.pointers, overworldDrawer.unSortedPointers);
                        else
            ...
            else
            {
                if (usedspace > freespace)
                {
                    ...
                    {
                        if (!cSprite.Checked)
                            save = mapSaver.saveDungeonCollision(dungeonDrawer.objects, dungeonDrawer.warps, dungeonIndex, mapIndex, floorTile, wallTiles, usedspace, freespace, dungeonDrawer.pointers, dungeonDrawer.unSortedPointers, cMagGlass.Checked);
                        else
                            save = mapSaver.saveSprites(sprites.spriteList, overWorld, mapIndex, dungeonIndex, usedspace, freespace, sprites.pointers, sprites.unSortedPointers);
                        if (save)
                        {
        ...
        private void repointCollisionsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (pTiles.Image != null)
            {
                ...
                RepointCollision RC = new RepointCollision(backup, overWorld, dungeonIndex, mapIndex, address, cSpecialTiles.Checked, cMagGlass.Checked, cSprite.Checked, warps, objects, wallTiles, floorTile);
                RC.ShowDialog();
